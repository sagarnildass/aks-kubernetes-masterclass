...
az aks get-credentials --resource-group artelus-aks-cluster-rg --name artelus-cluster-azure --file ~/.kube/aks-config --overwrite-existing

# Now as we have both aws and azure, we have to merge them
cp ~/.kube/config ~/.kube/eks-config-backup

KUBECONFIG=~/.kube/config:~/.kube/aks-config kubectl config view --merge --flatten > ~/.kube/combined-config

mv ~/.kube/combined-config ~/.kube/config

kubectl config get-contexts

# Switch to AWS context
kubectl config use-context admin-sagarnil@artelus-cluster.ap-south-1.eksctl.io

# Switch to Azure context
kubectl config use-context artelus-cluster-azure
